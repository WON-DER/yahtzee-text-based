class ScoreKeeper:
    ...
    def score_bonus(self):
        ...
        if (
            sum(
                ...
                if category in self.scord_board_upper_list
            )
            >= 63
        ):
        ...
        if self.score_board_dict["yahtzee bonus"] is None:
            self.score_board_dict["yahtzee bonus"] = 0
    ...
    def end_of_game_score(self):
        ...
        self.score_final = sum(
            score for category, score in self.score_board_dict.items()
        )
    ...
    def display_available_categories(self):
        ...
        for category, score in self.score_board_dict.items():
            if score is None and category not in ["yahtzee bonus", "upper bonus"]:
